	...
	make is
		...
		do
			...
			sqlgenerator.set_options(
				...
				TimeZoneEnabled,
				ViewsEnabled)
			if primary_key_format /= Void then
				sqlgenerator.set_primary_key_format (primary_key_format)
			end
			if sequence_name_format /= Void then
				sqlgenerator.set_sequence_name_format (sequence_name_format)
			end
			...
			if XMLEnabled then
				create {XML_GENERATOR} mwgenerator
			...
			if parser.syntax_error then
				...
				if mwgenerator /= Void then
					if XMLEnabled then
						mwgenerator.dump_statements (parser.statements, sqlgenerator)
					end
				end
			end
		end
