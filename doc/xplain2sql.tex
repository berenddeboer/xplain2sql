...
\hbox{
  \hbox{\vtop{\hsize 5.5cm\rightskip=0pt plus 1fill\baselineskip=24pt%
      \ss\bfd xplain2sql 3.0 manual\par\vskip12pt%
      \bfa 2009-December-02}}%
...
(see \publication[Bekke92]) some guidelines to convert Xplain to SQL
...
xplain2sql compiles on both Unix (tested on Ubuntu 9.10 and FreeBSD
6.4) and Windows (tested on Windows 2000 sp2). Any platform with a
...
1.2r7 or \index{ISE Eiffel}ISE Eiffel 6.2 or later. Earlier
...
have \index{Gobo}Gobo 3.5 or later installed on your system.
...
\VL \VL\LR
\VL -mysql5
\VL Output MySQL 5.0 (tested against version 5.0.51a).\VL\SR
\HL
\VL -mysql4
...
\VL \VL\LR
\VL -pgsql81
\VL Output PostgreSQL 8.1 sql (tested against version 8.2.7). \VL\SR
\HL
\VL -pgsql73
...
(see \publication[Gamma95]) is used a lot. Simple examples are the
...
implemented as a Singleton (see \publication[Gamma95]). This class
...
As suggested by BM in \publication[Meyer97] (chapter 24.3) the handle
...
pattern (see \publication[Gamma95]), but it would be a real Bridge if
...
\publication[Bekke92]. Xplain also refers to the Xplain product, a full
...
\publication[Bekke92]. I absolutely recommend this book, because it not
...
In \publication[Bekke96] a short introduction to Xplain is given,
...
Smith and Smith (\publication[Smith77a], \publication[Smith77b],
\publication[Smith78]). These abstractions are considered to be
...
In chapter 6 of \publication[Bekke92], a simple Xplain data model is
...
from \publication[Bekke92]) and demonstrate a feature of Xplain:
...
In \publication[Bekke92] three case studies are given. Parts of the
...
\startpublicatie[Bekke96]
  \naam{J.H.~ter~Bekke}
  \titel{Semantic modeling of successive events applied to version management}
  \jaar{1996}
  \plaats{International Symposium on Cooperative Database Systems
  for Advanced Applications, volume 1}
  \uitgever{Heian Shrine, Kyoto}
\stoppublicatie

\startpublicatie[Bekke97]
  \naam{J.H.~ter~Bekke}
  \titel{Can we rely on SQL?}
  \jaar{1997}
  \plaats{Proceedings 8th international DEXA workshop, 378-383}
  \plaats{Toulouse}
  \uitgever{IEEE Computer Society}
\stoppublicatie
...
\startpublicatie[Gamma95]
  \naam{Eric~Gamma, e.a.}
  \titel{Design Patterns}
  \jaar{1995}
  \plaats{Massachusetts}
  \uitgever{Addison-Wesley Publishing Company}
\stoppublicatie

\startpublicatie[Meyer97]
  \naam{Bertrand Meyer}
  \titel{Object-oriented software construction, 2nd edition}
  \jaar{1997}
  \plaats{New Jersey}
  \uitgever{Prentice-Hall, Inc.}
\stoppublicatie

\startpublicatie[Smith77a]
  \naam{J.M.~Smith and D.C.P.~Smith}
  \titel{Database abstractions: aggregation}
  \jaar{1977}
  \uitgever{Communications of the ACM 6, pp. 405-413}
\stoppublicatie

\startpublicatie[Smith77b]
  \naam{J.M.~Smith and D.C.P.~Smith}
  \titel{Database abstractions: aggregation and generalization}
  \jaar{1977}
  \uitgever{ACM Transactions on Database Systems 2, pp. 105-133}
\stoppublicatie

\startpublicatie[Smith78]
  \naam{J.M.~Smith}
  \titel{Principals of database conceptual design}
  \jaar{1978}
  \uitgever{Proceedings NYU Symposium on database design, pp. 35-49}
\stoppublicatie
